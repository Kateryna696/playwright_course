...
export default class CarsController {
    ...
    async getCarById(id: number, cookies: string) {
        const response = await this.request.get(`/api/cars/${id}`, {
            headers: {
                'Cookie': `sid=${cookies}`
            },
    ...
    async editCar(brandId: number, modelId: number, mileage: number, cookies: string, id: number) {
        const response = await this.request.put(`/api/cars/${id}`,
            {
                data: {
                    "carBrandId": brandId,
                    "carModelId": modelId,
